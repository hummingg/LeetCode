...
# @lc code=start
class Solution:
    # 73: 28.63%(116ms) 46.89%(15MB);25.73%(128ms) 95.44%(14.8MB);
    def longestNiceSubstring(self, s: str) -> str:
        size = len(s)
        ret = ''
        for i in range(size):
            cnt = [[0, 0] for _ in range(26)]   # 深拷贝
            for j in range(i, size):
                if s[j] < 'a':
                    cnt[ord(s[j])+32-97][1] += 1
                else:
                    cnt[ord(s[j])-97][0] += 1
                if self.isBeautiful(cnt):
                    if len(ret) < len(s[i:j+1]):
                        ret = ''.join(s[i:j+1])
        return ret
                
    def isBeautiful(self, cnt: list) -> bool:
        for i in range(26):
